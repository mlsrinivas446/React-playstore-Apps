...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, searchApps: ''}

  appSearch = event => {
    this.setState({searchApps: event.target.value})
  }
  ...
  render() {
    const filteredAppList = this.filteredTadItems()
    const {activeTabId, searchApps} = this.state

    const resultApps = filteredAppList.filter(each =>
      each.appName.toLowerCase().includes(searchApps.toLowerCase()),
    )
        ...
        <ul className="tab-items-container">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              tabsList={each}
              updateTadItems={this.updateTadItems}
